...
int edges;
int last[105],prev[5200],head[5200],cap[5200],flow[5200],dist[105];
int nextedge[105]; //used for keeping track of next edge of ith node
...
int main()
{
    int M,W,source,sink,cost,u,v,i,x,y;
    ...
    while(sc2(M,W))
    {
        ...
        //for boss machine
        addedge(2*1-1,2*1,9999999,0);
        addedge(2*1,2*1-1,0,0);
        addedge(2*1-1,2*1,0,0);
        addedge(2*1,2*1-1,9999999,0);

        for(int i=2; i<=M-1; i++)
        {
            sc2(y,x);
            addedge(2*y-1,2*y,x,0);
            addedge(2*y,2*y-1,0,0);
            addedge(2*y-1,2*y,0,0);
            addedge(2*y,2*y-1,x,0);
        }

        //for server machine
        addedge(2*M-1,2*M,9999999,0);
        addedge(2*M,2*M-1,0,0);
        addedge(2*M-1,2*M,0,0);
        addedge(2*M,2*M-1,9999999,0);
        rep(i,W)
        {
            sc3(u,v,cost);
            addedge(2*u,2*v-1,cost,0);
            addedge(2*v-1,2*u,0,0);//bi-directional
            addedge(2*v,2*u-1,cost,0);
            addedge(2*u-1,2*v,0,0);//bi-directional
        }
        printf("%d\n",maxflow(1,2*M,2*M));
    }
    return 0;
