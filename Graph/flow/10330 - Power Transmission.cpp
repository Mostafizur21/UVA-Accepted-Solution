...
int edges;
int last[202],prev[20001],head[20001],cap[20001],flow[20001],dist[202];
int nextedge[202]; //used for keeping track of next edge of ith node
...
int main()
{
    int n,M,source,sink,cost,u,v,i,x,B,D;
    ...
    while(sc1(n)!=EOF)
    {
        ...
        rep(i,M)
        {
            sc3(u,v,cost);
            u--,v--;//0 based
            addedge(u*2+1,v*2,cost,0);
            addedge(v*2,u*2+1,0,0);//bi-directional
        }
        sc2(B,D);
        rep(i,B)
        {
            sc1(x);
            x--;
            addedge(source,x*2,9999999,0);
        }
        rep(i,D)
        {
            sc1(x);
            x--;
            addedge(x*2+1,sink,9999999,0);
        }
        printf("%d\n",maxflow(source,sink,cur+1));
    }
    return 0;
}
